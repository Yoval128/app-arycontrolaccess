...
const EditUserScreen = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            navigation.replace('ListUsers', {refresh: true});
        } catch (error) {
                    ...
                    {loading ? (
                        ...
                        <VStack space={4}>
                            ...
                            <FormControl>
                                ...
                                <Dropdown data={estado} labelField="label" valueField="value"
                                          value={user.Estado}
                                          onChange={(item) => setUser({...user, Estado: item.value})}
                                />
                            </FormControl>

                            <Button colorScheme="secondary" mt={5} isLoading={saving} onPress={handleSave}
                                    leftIcon={<Ionicons name="save-outline" size={20} color="white"/>}>Guardar
                                Cambios</Button>
                            <Button mt={2} onPress={() => navigation.goBack()} variant="outline" colorScheme="danger"
                                    leftIcon={<Ionicons name="close" size={20} color="red"/>}>Cancelar</Button>
                        </VStack>
                    )}
                </Box>
            </ScrollView>
